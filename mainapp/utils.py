...
def create_db_config(application):
    ...
    if config.getboolean("testing", "testing"):
        ...
        application.config['SQLALCHEMY_POOL_SIZE'] = config.getint("database", "pool_size")
    if config.getboolean("testing", "testing"):
        application.config['SQLALCHEMY_POOL_TIMEOUT'] = None
    else:
        application.config['SQLALCHEMY_POOL_TIMEOUT'] = config.getint("database", "pool_timeout")
    application.config['SQLALCHEMY_POOL_RECYCLE'] = config.getint("database", "pool_recycle")
